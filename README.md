...
1. What is the DOM?
   DOM is Document Objet Model where we can access nodes and make changes to the DOM however we like.

2. What is an event?
   Event is something expected to happen. We make addEventListeners to "listen" to clicks dblclicks or even entering a keypress to signal over to the data that once a user does this return with whatever it is listening for.

3. What is an event listener?
   addEventListener is an action that listens to whatever we give it. If we give it wheel functionality it'll wait for a wheel to happen and if it does return whatever you wanted to happen happen.

4. Why would we convert a NodeList into an Array?
   It's easier to manipulate. With an array you can use .this statements to pull out necessary data you need.

5. What is a component?
   Components are reusuable code. It's basically same as a javascript function but work independetly to return HTML information.
